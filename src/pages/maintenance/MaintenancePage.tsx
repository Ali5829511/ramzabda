...
import {
  ...
  Zap, ClipboardList, MapPin, Hash, ChevronRight,
  FileCheck, Receipt, PlusCircle, Minus
} from 'lucide-react';
...
function AddQuoteInline({ req, onUpdate, now, addHistory, currentUser }: {
  ...
  const submit = () => {
    ...
    const quote: PriceQuote = {
      ...
      ownerApproval: req.ownerApprovalStatus === 'approved' ? 'not_required' : 'pending',
  ...
  const handleSubmit = () => {
    ...
    if (hasQuote && quoteItems.some(it => it.description)) {
      quote = {
        ...
        ownerApproval: needsOwnerApproval ? 'pending' : 'not_required',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 flex-wrap border-b border-gray-200 pb-1">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as 'dashboard' | 'all' | 'by_property' | 'approval' | 'technician' | 'tenant' | 'owner')}
