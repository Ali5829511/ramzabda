...
import {
  ...
  Clock, AlertTriangle, Search, User, Building2,
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <span key={key}
                  className={`text-xs px-3 py-1.5 rounded-full font-medium ${(c as unknown as Record<string, boolean>)[key] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400 line-through'}`}>
                  {icon} {label}
                </span>
              ))}
            </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {[
                        ...
                        <label key={key} className={`flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all ${(form as unknown as Record<string, boolean>)[key] ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                          <input type="checkbox" checked={(form as unknown as Record<string, boolean>)[key]} onChange={e => setForm({ ...form, [key]: e.target.checked })} className="w-4 h-4 accent-yellow-500" />
