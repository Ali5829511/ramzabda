...
export default function DocumentArchivePage() {
  ...
  const in90 = useMemo(() => new Date(new Date().getTime() + 90 * 86400000).toISOString().slice(0, 10), []);
  ...
  const filtered = docs.filter(d => {
    ...
    return (
      ...
      (!filterExpiry || (filterExpiry === 'expired' ? (d.expiresAt ?? '9999') < today : filterExpiry === 'expiring' ? (d.expiresAt ?? '9999') < in90 && (d.expiresAt ?? '') >= today : true))
    );
  });
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          {filtered.length === 0 ? (
            ...
            const isExpiring = !isExpired && doc.expiresAt && doc.expiresAt < in90;
            return (
