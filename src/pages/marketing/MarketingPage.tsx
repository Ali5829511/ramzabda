...
import {
  ...
  TrendingUp, Star, BarChart2, Calendar, Clock, CheckCircle,
  User, Phone, Building2, DollarSign, Target, Zap,
  Play, Pause, X, Search, Copy, Home, Tag
} from 'lucide-react';
              ...
              <div className="grid grid-cols-4 gap-2">
                {(['impressions','clicks','leads','conversions'] as const).map(f => (
                  <div key={f}>
                    ...
                    <input type="number" min="0" className="input-field" value={(form as Record<string, unknown>)[f] as number||''} onChange={e => setForm({...form, [f]: +e.target.value})} />
                  </div>
                ))}
              </div>
              <div className="flex gap-3 pt-2">
